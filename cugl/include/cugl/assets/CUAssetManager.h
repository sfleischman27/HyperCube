...
#pragma mark Loader Management
     ...
     *
     * If the loader has a nonempty JSON key, then it will support asset
     * directories.  It its JSON key is already in use, then it will not
     * its JSON key will be set to empty
     *
    ...
    bool attach(const std::shared_ptr<BaseLoader>& loader) {
        ...
        std::string key = loader->getJsonKey();
        Uint32 rank = loader->getPriority();
        ...
        // Do not allow key collisions
        auto other = _jsonKeys.find(key);
        if (other != _jsonKeys.end()) {
            key = "";
            loader->setJsonKey(key);
        }
    ...
    bool detach() {
        ...
        std::string key = it->second->getJsonKey();
        it->second = nullptr;
...
#pragma mark Directory Support
     ...
     * restricted by the loaders that the programmer provides.
     *
