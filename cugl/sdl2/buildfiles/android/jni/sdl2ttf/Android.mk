...
#
###########################
SDL2_TTF_PATH := $(SDL2_PATH)/extensions/ttf

# Enable this if you want to use HarfBuzz
SUPPORT_HARFBUZZ ?= true
HARFBUZZ_LIBRARY_PATH := $(SDL2_TTF_PATH)/external/harfbuzz

FREETYPE_LIBRARY_PATH := $(SDL2_TTF_PATH)/external/freetype

# Build freetype library
ifneq ($(FREETYPE_LIBRARY_PATH),)
    include $(FREETYPE_LIBRARY_PATH)/Android.mk
endif

# Build the library
ifeq ($(SUPPORT_HARFBUZZ),true)
    include $(HARFBUZZ_LIBRARY_PATH)/Android.mk
endif

# Restore local path
LOCAL_PATH := $(SDL2_TTF_PATH)
include $(CLEAR_VARS)
...
ifeq ($(SUPPORT_HARFBUZZ),true)
    LOCAL_C_INCLUDES += $(HARFBUZZ_LIBRARY_PATH)/src
