...
def place_project(config):
    ...
    # Pick the right JNI file
    jnidir = os.path.join(project,'app','jni','sdl2core')
    jniNix = os.path.join(jnidir,'Android.nix')
    jniWin = os.path.join(jnidir,'Android.win')
    jniMK = os.path.join(jnidir,'Android.mk')
    if any(platform.win32_ver()):
        shutil.move(jniWin, jniMK)
        os.remove(jniNix)
    else:
        shutil.move(jniNix, jniMK)
        os.remove(jniWin)
...
def config_make(config,project):
    """
    ...
    # Modify the SDL2 makefile
    sdlmake = os.path.join(project,'app','jni','sdl2core','Android.mk')
    sdldir  = os.path.join(*prefix,config['build_to_sdl2'],'buildfiles','android','jni')
    sdldir  = util.path_to_posix(sdldir)
    contents = {'__SDL2_PATH__':sdldir}
    util.file_replace(sdlmake,contents)
...
def make(config):
    ...
    print('-- Modifying NDK makefiles')
    config_make(config,project)
