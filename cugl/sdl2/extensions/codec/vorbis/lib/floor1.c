...
int *floor1_fit(vorbis_block *vb,vorbis_look_floor1 *look,
  ...
  if(posts==0){
    nonzero+=accumulate_fit(logmask,logmdct,0,(int)n,fits,(int)n,info);
  }else{
    for(i=0;i<posts-1;i++)
      nonzero+=accumulate_fit(logmask,logmdct,(int)look->sorted_index[i],
                              (int)look->sorted_index[i+1],fits+i,
                              (int)n,info);
  }
  ...
  if(nonzero){
    ...
    int y0=-200;
    int y1=-200;
    fit_line(fits,(int)(posts-1),&y0,&y1,info);
    ...
    for(i=2;i<posts;i++){
      ...
      int vx=post_Y(fit_valueA,fit_valueB,(int)i);
      ...
      if(csubbits){
        ...
        for(k=0;k<csub;k++){
          ...
          if(booknum<0){
            ...
            maxval[k]=(int)sbooks[info->class_subbook[class][k]]->entries;
          }
        }
        for(k=0;k<cdim;k++){
          for(l=0;l<csub;l++){
...
static void *floor1_inverse1(vorbis_block *vb,vorbis_look_floor *in){
  ...
  if(oggpack_read(&vb->opb,1)==1){
    ...
    fit_value[0]=(int)oggpack_read(&vb->opb,ov_ilog(look->quant_q-1));
    fit_value[1]=(int)oggpack_read(&vb->opb,ov_ilog(look->quant_q-1));
    ...
    for(i=0,j=2;i<info->partitions;i++){
      ...
      if(csubbits){
        cval=(int)vorbis_book_decode(books+info->class_book[class],&vb->opb);
