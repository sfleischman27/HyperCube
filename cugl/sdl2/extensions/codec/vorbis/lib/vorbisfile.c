...
static int _open_seekable2(OggVorbis_File *vf){
  ...
  end=_get_prev_page_serial(vf,vf->end,vf->serialnos+2,(int)vf->serialnos[1],&endserial,&endgran);
  if(end<0)return((int)end);

  /* now determine bitstream structure recursively */
  if(_bisect_forward_serialno(vf,0,dataoffset,end,endgran,endserial,
                              vf->serialnos+2,(int)vf->serialnos[1],0)<0)return(OV_EREAD);
...
int ov_raw_seek(OggVorbis_File *vf,ogg_int64_t pos){
  ...
  ogg_stream_reset_serialno(&vf->os,
                            (int)vf->current_serialno); /* must set serialno */
  ...
  */

  {
    ...
    ogg_stream_init(&work_os,(int)vf->current_serialno); /* get the memory ready */
    ...
    while(1){
      ...
      if(vf->ready_state<STREAMSET){
        ...
        vf->current_link=link;
        vf->current_serialno=serialno;
        ogg_stream_reset_serialno(&vf->os,(int)serialno);
        ogg_stream_reset_serialno(&work_os,(int)serialno);
        vf->ready_state=STREAMSET;
