...
int vorbis_staticbook_pack(const static_codebook *c,oggpack_buffer *opb){
  ...
  if(ordered){
    ...
    for(i=1;i<c->entries;i++){
      ...
      if(this>last){
        for(j=last;j<this;j++){
          oggpack_write(opb,i-count,ov_ilog((int)(c->entries-count)));
          count=i;
        }
      }
    }
  ...
  case 1:case 2:
...

    {
      ...
      case 1:
        ...
        quantvals=(int)_book_maptype1_quantvals(c);
        break;
      case 2:
        /* every value (c->entries*c->dim total) specified explicitly */
        quantvals=(int)(c->entries*c->dim);
        break;
...
static_codebook *vorbis_staticbook_unpack(oggpack_buffer *opb){
  ...
  if(ov_ilog((int)s->dim)+ov_ilog((int)s->entries)>24)goto _eofout;
  ...
  case 1:
    /* ordered */
    {
      ...
      for(i=0;i<s->entries;){
        long num=(int)oggpack_read(opb,ov_ilog((int)(s->entries-i)));
        if(num==-1)goto _eofout;
  ...
  case 1: case 2:
    ...
    s->q_min=(int)oggpack_read(opb,32);
    s->q_delta=(int)oggpack_read(opb,32);
...

    {
      ...
      switch(s->maptype){
      case 1:
        quantvals=(s->dim==0?0:(int)_book_maptype1_quantvals(s));
        break;
      case 2:
