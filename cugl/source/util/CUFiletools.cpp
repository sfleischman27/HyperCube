 ...
 */
static std::string to_absolute(std::string path) {
...
Uint64 file_timestamp(const std::string path) {
    ...
    HANDLE handle = FindFirstFileW(wide.c_str(), &search_data);
...
std::vector<std::string> dir_contents(const std::string path) {
    ...
    while (avail && handle != INVALID_HANDLE_VALUE) {
        ...
        avail = (FindNextFileW(handle, &search_data) != FALSE);
    }
...
size_t vol_free_space(const std::string path) {
    ...
    std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> converter;
    std::wstring wide = converter.from_bytes(fullpath);
    bool okay = GetDiskFreeSpaceExW(wide.c_str(), &freeBytesAvail, &totalNumBytes, &totalBytesAvail);
    if (okay) {
