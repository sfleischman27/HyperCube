...
bool AudioNode::init(Uint8 channels, Uint32 rate) {
    if (_booted) {
        ...
        CUAssertLog(false,"Attempt to allocate a node without an active audio device manager");
        return false;
    }
    _channels = channels;
    _sampling = rate;
    ...
    if (AudioEngine::get() != nullptr) {
        _readsize = AudioEngine::getReadSize();
    } else {
        _readsize = AudioDevices::DEFAULT_READ_SIZE;
    }
    return true;
}
 ...
 */
void AudioNode::setReadSize(Uint32 size) {
    _readsize = size;
}
