...
bool AudioInput::init() {
    ...
    Uint32 read = AudioDevices::get()->getReadSize();
    Uint32 write = AudioDevices::get()->getWriteSize();
    return init("",DEFAULT_CHANNELS,DEFAULT_SAMPLING,read,write);
}
...
bool AudioInput::init(Uint8 channels, Uint32 rate) {
    ...
    Uint32 read = AudioDevices::get()->getReadSize();
    Uint32 write = AudioDevices::get()->getWriteSize();
    return init("",channels,rate,read,write);
}
 ...
 * @return true if initialization was successful
 */
bool AudioInput::init(Uint8 channels, Uint32 rate, Uint32 readsize, Uint32 writesize) {
    return init("",channels,rate,readsize,writesize);
}
...
bool AudioInput::init(const std::string device) {
    ...
    Uint32 read = AudioDevices::get()->getReadSize();
    Uint32 write = AudioDevices::get()->getWriteSize();
    return init(device,DEFAULT_CHANNELS,DEFAULT_SAMPLING,read,write);
}
 ...
 * @return true if initialization was successful
 */
bool AudioInput::init(const std::string device, Uint8 channels, Uint32 rate,
...
bool AudioInput::reopenDevice() {
    ...
    _device = SDL_OpenAudioDevice((_dvname == "" ? NULL : _dvname.c_str()),
                                  1, &_wantspec, &_audiospec, SDL_AUDIO_ALLOW_ANY_CHANGE);
 ...
 */
void AudioInput::allocateBuffer() {
    lock();
    
    Uint32 capacity = _capacity;    // Old capacity
    _writesize = _audiospec.samples;
    _capacity = _writesize+_readsize+_readdelay;

    float* buffer = (float*)malloc(_capacity*_channels*sizeof(float));
    if (buffer == nullptr) {
        _buffsize = _readsize+_readdelay;
        _buffhead = _writesize;
        _bufftail = 0;
        std::memset(_buffer,0,_capacity*_channels*sizeof(float));
    } else {
        Uint32 delay  = _capacity-_buffsize;
        Uint32 factor = _channels*sizeof(float);
        std::memset(buffer,0,delay*factor);
        if (_buffhead+_buffsize > capacity) {
            Uint32 amt = capacity-_buffhead;
            Uint32 offset = delay+amt;
            std::memcpy(buffer+delay*_channels,_buffer+_buffhead*_channels,amt*factor);
            std::memcpy(buffer+offset*_channels,_buffer,(_buffsize-amt)*factor);
        } else {
            std::memcpy(buffer+delay*_channels,_buffer+_buffhead*_channels,_buffsize*factor);
        }
        _buffsize = _capacity-delay;
        _buffhead = delay;
        _bufftail = 0;
        free(_buffer);
        _buffer = buffer;
 ...
 */
void AudioInput::setWriteSize(Uint32 size) {
    if (size != _writesize) {
        _wantspec.samples = _writesize;
        if (!reopenDevice()) {
            dispose();
        }
    }
 ...
 */
void AudioInput::setReadDelay(Uint32 size) {
    if (_readdelay != size) {
        _readdelay = size;
        allocateBuffer();
    }
 ...
 */
void AudioInput::setReadSize(Uint32 size) {
    if (_readsize != size) {
        _readsize = size;
        allocateBuffer();
    }
