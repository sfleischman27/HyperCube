...
bool AudioScheduler::init(Uint8 channels, Uint32 rate) {
    if (AudioNode::init(channels,rate)) {
        ...
        std::memset(_buffer,0,_readsize*_channels*sizeof(float));
        return true;
    }
    return false;
}
 ...
 */
void AudioScheduler::setReadSize(Uint32 size) {
    if (_readsize != size) {
        _readsize = size;
        if (_buffer != nullptr) {
            free(_buffer);
        ...
        std::shared_ptr<AudioNode> node = _current;
        if (node != nullptr) {
...
Uint32 AudioScheduler::read(float* buffer, Uint32 frames) {
    ...
    while (amt < frames && current != nullptr) {
        ...
        if (previous && current && overlap > 0) {
            ...
            amt += current->read(&(buffer[amt*_channels]),need);
            if (amt < frames || current->completed()) {
                // Time to check for a loop, or notify that we are done
                if (loop) {
                    if (!current->reset()) {
                        current = nullptr;
                        _current = nullptr;
...
bool AudioScheduler::mark() {
    // TODO: FINISH ME
    CUAssertLog(false, "Method currently unsupported.");
    return false;
}
...
std::shared_ptr<AudioNode> AudioScheduler::acquire(Sint32& loop, Uint32 skip, AudioNode::Action action) {
    ...
    if (result != nullptr && result->getReadSize() != _readsize) {
        result->setReadSize(_readsize);
    }
