...
AudioDevices* AudioDevices::_gManager = nullptr;

#if CU_PLATFORM == CU_PLATFORM_ANDROID
/** The default input buffer size for each audio node */
const Uint32 AudioDevices::DEFAULT_READ_SIZE = 2048;
#elif CU_PLATFORM == CU_PLATFORM_IPHONE
/** The default input buffer size for each audio node */
const Uint32 AudioDevices::DEFAULT_READ_SIZE = 1024;
#else
/** The default input buffer size for each audio node */
const Uint32 AudioDevices::DEFAULT_READ_SIZE = 512; 
#endif

/** The default input buffer size for each audio node */
const Uint32 AudioDevices::DEFAULT_WRITE_SIZE = 1024;
 ...
 */
bool AudioDevices::init(Uint32 output, Uint32 input) {
 ...
 */
void AudioDevices::start() {
    start(DEFAULT_READ_SIZE,DEFAULT_WRITE_SIZE);
}
 ...
 */
void AudioDevices::start(Uint32 frames) {
    start(frames,frames);
}
 ...
 */
void AudioDevices::start(Uint32 output, Uint32 input) {
    if (_gManager) {
        CUAssertLog(!_gManager, "Audio Manager is already in use");
        return;
    }
    _gManager = new AudioDevices();
 ...
 */
std::vector<std::string> AudioDevices::devices(bool output) {
 ...
 */
std::vector<std::string> AudioDevices::occupied(bool output) {
    if (!_gManager) {
 ...
 */
void AudioDevices::setReadSize(Uint32 size) {
    _output = size;
}
 ...
 * devices. You should call {@link AudioInput#setCapacity} instead.
 *
 * @param size  The size of the write buffer (in frames) for input nodes.
 */
void AudioDevices::setWriteSize(Uint32 size) {
    _input = size;
}
 ...
 */
std::shared_ptr<cugl::audio::AudioOutput> AudioDevices::openOutput() {
    std::unique_lock<std::mutex> lock(_mutex);
 ...
 */
std::shared_ptr<cugl::audio::AudioOutput> AudioDevices::openOutput(Uint8 channels, Uint32 rate) {
    std::unique_lock<std::mutex> lock(_mutex);
    if (_outputs.find("") != _outputs.end()) {
        CULogError("Default output device is in use.");
        return nullptr;
 ...
 */
std::shared_ptr<audio::AudioOutput> AudioDevices::openOutput(const char* device) {
    std::unique_lock<std::mutex> lock(_mutex);
 ...
 * @return the given output device with 2 channels at 48000 Hz.
 */
std::shared_ptr<audio::AudioOutput> AudioDevices::openOutput(const std::string device) {
    std::unique_lock<std::mutex> lock(_mutex);
 ...
 */
std::shared_ptr<audio::AudioOutput> AudioDevices::openOutput(const char* device, Uint8 channels, Uint32 rate) {
    std::unique_lock<std::mutex> lock(_mutex);
    std::string name(device);
    if (_outputs.find(name) != _outputs.end()) {
        CULogError("Device '%s' is in use.",device);
 ...
 * @return the output device with the given channels and sample rate.
 */
std::shared_ptr<audio::AudioOutput> AudioDevices::openOutput(const std::string device, Uint8 channels, Uint32 rate) {
    std::unique_lock<std::mutex> lock(_mutex);
    if (_outputs.find(device) != _outputs.end()) {
        CULogError("Device '%s' is in use.",device.c_str());
        return nullptr;
 ...
 */
bool AudioDevices::closeOutput(const std::shared_ptr<audio::AudioOutput>& output) {
    std::unique_lock<std::mutex> lock(_mutex);
 ...
 */
std::shared_ptr<cugl::audio::AudioInput> AudioDevices::openInput() {
    std::unique_lock<std::mutex> lock(_mutex);
 ...
 */
std::shared_ptr<cugl::audio::AudioInput> AudioDevices::openInput(Uint8 channels, Uint32 rate, Uint32 delay) {
    std::unique_lock<std::mutex> lock(_mutex);
    if (_inputs.find("") != _inputs.end()) {
        CULogError("Default output device is in use.");
        return nullptr;
 ...
 */
std::shared_ptr<audio::AudioInput> AudioDevices::openInput(const char* device) {
    std::unique_lock<std::mutex> lock(_mutex);
    std::string name(device);
 ...
 * @return the given input device with 2 channels at 48000 Hz.
 */
std::shared_ptr<audio::AudioInput> AudioDevices::openInput(const std::string device) {
    std::unique_lock<std::mutex> lock(_mutex);
 ...
 */
std::shared_ptr<audio::AudioInput> AudioDevices::openInput(const char* device, Uint8 channels, Uint32 rate, Uint32 delay) {
    std::unique_lock<std::mutex> lock(_mutex);
    std::string name(device);
    if (_inputs.find(name) != _inputs.end()) {
        CULogError("Device '%s' is in use.",device);
 ...
 * @return the given output device with the given channels and sample rate.
 */
std::shared_ptr<audio::AudioInput> AudioDevices::openInput(const std::string device, Uint8 channels, Uint32 rate, Uint32 delay) {
    std::unique_lock<std::mutex> lock(_mutex);
    if (_inputs.find(device) != _inputs.end()) {
        CULogError("Device '%s' is in use.",device.c_str());
        return nullptr;
 ...
 * @return whether the device was successfully closed.
 */
bool AudioDevices::closeInput(const std::shared_ptr<audio::AudioInput>& input) {
    std::unique_lock<std::mutex> lock(_mutex);
