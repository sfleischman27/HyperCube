...
bool Display::init(std::string title, Rect bounds, Uint32 flags) {
    ...
    _display = 0;
    SDL_Rect native;
    _scale = SDL_GetDisplayPixelDensity(_display);

    if (flags & INIT_FULLSCREEN) {
        SDL_GetDisplayPixelBounds(_display,&native);
        _fullscreen = true;
        SDL_ShowCursor(0);
    ...
    _notched = SDL_CheckDisplayNotch(_display);
...
// The mobile devices have viewport problems
    ...
    _deviceOrientation  = translateOrientation(SDL_GetDeviceOrientation(_display));
    _defaultOrientation = translateOrientation(SDL_GetDefaultOrientation(_display));
    return true;
}
