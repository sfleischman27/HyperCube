...
def config_make(config,project):
    ...
    # Modify the auxiliary makefiles
    cugldir = os.path.join(*prefix,config['build_to_cugl'])
    cugldir  = util.path_to_posix(cugldir)
    contents = {'__CUGL_PATH__':cugldir}
    
    cuglmake = os.path.join(project,'app','jni','cugl','Android.mk')
    util.file_replace(cuglmake,contents)
    box2make = os.path.join(project,'app','jni','box2d','Android.mk')
    util.file_replace(box2make,contents)
    geommake = os.path.join(project,'app','jni','geomtools','Android.mk')
    util.file_replace(geommake,contents)
    sdl2make = os.path.join(project,'app','jni','sdl2core','Android.mk')
    util.file_replace(sdl2make,contents)
