...
def place_project(config):
    ...
    template = os.path.join(config['cugl'],'templates','apple','app.xcodeproj')
    ...
    src = os.path.join(config['cugl'],'templates','apple','Resources')
...
def parse_pbxproj(config,project):
    """
...
def reassign_pbxproj(config,pbxproj):
    ...
    sdl2dir = '../'+util.path_to_posix(os.path.join(config['build_to_cugl'],'sdl2'))
    geomdir = '../'+util.path_to_posix(os.path.join(config['build_to_cugl'],'thirdparty','geomtools'))
    box2dir = '../'+util.path_to_posix(os.path.join(config['build_to_cugl'],'thirdparty','box2d'))
    ...
    for pos in range(len(section)):
        if 'cugl.xcodeproj' in section[pos]:
            ...
            section[pos] = entry[:pos0]+'path = '+cugldir+'/buildfiles/apple/cugl.xcodeproj'+entry[pos1:]
    ...
    for pos in range(len(section)):
        ...
        if '__GEOM_INCLUDE__' in section[pos]:
            section[pos] = section[pos].replace('__GEOM_INCLUDE__','"$(SRCROOT)/'+geomdir+'/include"')
...
def make(config):
    ...
    pbxproj = parse_pbxproj(config,project)
